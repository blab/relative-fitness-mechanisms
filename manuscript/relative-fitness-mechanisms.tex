...
% TB: As below, I've adjusted text to $\varEscape=0.05$ based on my understanding of "Two-Strain SIR"
...
\begin{figure}[h]
    ...
    \caption{
      ...
      We introduce the transmissibility variant at $t=20$ with frequency $f_T(20) = 10^{-5}$ and transmissibility increase $\varTransmission_T = 0.5$.
      We introduce the escape variant at $t=70$ with frequency $f_E(70) = 10^{-6}$ and $\varEscape_E = 0.05$.
      % TB: Looking at "Two-Strain SIR" I believe that $\varEscape$ should range from 0 (no escape) to 1 (hosts recovered to wild-type are just as susceptible as naive hosts). I've adjusted text accordingly.
...
\end{align*}
% TB: I'm confused here. I thought that variant transmissibility = wildtype transmissibility when $\varTransmission = 0$. However, in the second equation here it's clear that we get $\lambda_{\varT, \wt}$ when $\varTransmission = 1$. Ah, looking at ``Two-Strain SIR'' I see that it's clearly $\varTransmission \beta$. I've fixed text above and made a note about interpreting these parameter values.}
...
\begin{figure}[h]
    ...
    \caption{
      ...
      A. Relative fitness for a transmissibility increasing variant $T$ and an immune escaping variant $E$ for $\varTransmission_T=0.2$, $\varEscape_E=0.3$, $\beta=0.875$.
...
\tbc{This reads like a note to yourself. You need to signficantly expand this and reference evidence in the form of figures or Supplemental Text.}
...
Though it is useful to quantify the relative fitnesses of individual variants, we are often interested in quantifying the overall effects of selection in the population.
...
High fitness variants rising from initial low frequency leads to large increases in the variance of the fitness distribution and therefore increases in the selective pressure.

The selective pressure metric enables us to decompose changes in the average growth rate in the population to an evolutionary component $\psi$ and a residual baseline growth rate $r_0$:

\begin{equation}
    \frac{d\bar{r}}{dt} = \frac{dr_0}{dt} + \psi(t).
\end{equation}
...
\begin{align}
    \bar{r}(t) &= \bar{r}(0) + [r_0(t) - r_0(0)] + \Psi(t)\\
    \frac{I(t)}{I(0)} &= \exp(\int_0^t [r_0(s) + \Psi(s)]ds).
\end{align}

In addition to estimating the relative fitness, metrics derived from these models can be informative of much more.
Our ``selective pressure'' metric which allows us to model the contribution of evolution to changes in the epidemic growth rate of a population.
...
The full derivation of this metric and its contribution to the overall growth rate can be found in Supplemental Appendix \ref{ssec:deriving_selective_pressure}.

% \tbc{With Main Text you want to describe `results', but not go into too much detail on how they were arrived at. In the most typical case you present a Figure in Main Text and describe a bit about how it was derived, but with most details in Methods. I'd suggest thinking of the math here in a similar fashion. You have the Equation 3 result (and the two equations following) as big picture understanding of selective pressure. Good to include this in Main Text. But much of the preceeding derivation of Equation 3 can probably be moved to Methods or Supplemental Text.}
% MF: Tried to address this by moving most to the supplemental text.

\subsection*{Predicting epidemic growth rates using selective pressure}

Motivated by the relationship between the epidemic growth rate and the selective pressure demonstrated above, we develop a predictive model of the epidemic growth rate using estimates of selective pressure.
...
Using case and sequence data from 50 US states between March 2021 and September 2022, we estimate epidemic growth rates in these states and as well as selective pressure using our approximate Gaussian model on sequence counts grouped at the level of Nextstrain clades. 
We use these estimates to fit a gradient-boosted regressor to predict epidemic growth rates using selective pressure from the most recent 30 day, reserving data between March 2022 and September 2022 for testing (Fig.~\ref{fig:selective_pressure_prediction} A--F, Fig.~\ref{fig:empirical_growth_rate_predictions_all}).

%TODO: Update figure panels for Selective pressure figure
% \tbc{More text about what's going on. You looked at case counts? What are the variants here? Are they Pango lineages, Nextstrain clades? or something else? Some of this can be clarified visually in my proposal of panel directly plotting variant frequencies.}
\tbc{Statement on predictive performance here.}
Our model is able to replicate patterns seen in estimates of the epidemic growth rates in England derived from ONS data between Jan 2022 and Sep 2022 (Fig.~\ref{fig:selective_pressure_prediction} G--I), achieving a Spearman correlation of $R^2 =$.

%TODO: Describe ONS data and find relevant citations.
...
\begin{figure}[h]
    ...
    \caption{
      ...
      \tbc{This is currently a little too much magic / too divorced from intuition. I'd suggest a top set of panels that show a single state (whichever you'd like). With one panel showing variant frequencies in this state. Another panel showing case counts in this state. And a third panel showing selective pressure in this state. This then gives a bridge to the more derived information in the current figure.}
      \tbc{Be consistent with the semantics of your colors. Currently gray means both regressor model on selected pressure and ONS prevalence.}
      %MF: Grey here is ONS Prevalence + the epidemic growth rate from data. The regressor is green. Will make sure I'm consistent going forward with edits.
    }
...
These 14 countries were chosen based on availability of sequences and the 41 variants were chosen, so that each variant met a threshold for number of sequences available.
% \tbc{Which period? Again, what are the variants? Why these 14 countries?}


From these sequence counts, we use apply our latent factor model to estimate the relative fitness of each variant over time in each country, pseudo-escape rates for each variant, and pseudo-immunity for each country simultaneously for $K=5$ pseudo-immune groups.
...
Further, our model shows that these groups of lineages tend to target particular immune groups such as Clade 24A (JN.1, JN.1.1, JN.1.4) has high pseudo-escape in dimension 2.
...
We, then, compare these distances to distances in our pseudo-escape space (Fig.~\ref{fig:latent_immune}G), finding the distances in the pseudo-escape space are correlated with these titer differences between variants ($R^2$ = 0.6).
...
\begin{figure}[h]
    ...
    \caption{
      ...
      \tbc{Relative fitness to what? Note in the y axis label.}
      ...
      %MF: For identifiablity these are difference relative to the pivot variant, so it's fine for them to be negative
