...
Further, when mechanistic grounding has been considered, these modeling approaches have relied on molecular or serological data that is rarely available in real-time due to the speed and cost of experiments \cite{Meijers2023, Raharinirina2025}. 
...
Finally, we develop a latent immunity model that we use to estimate the underlying proportion of pseudo-immune groups within multiple geographies and pseudo-immune escape rates for circulating variants which recreates antigenic distances using sequence data alone.
...
\end{align}
where $\beta$ is the transmissibility coefficient, $\varEscape$ is escape proportion against the wildtype, $\varTransmission$ is the variant's proportional increase to transmissibility.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\linewidth]{./figures/latent-immune-structure.png}
    \caption{
...
Our learned immune clusters yields clear separation (Fig.~\ref{fig:latent_immune_structure}A) in PCA-space, whereas as reported exposure history shows broader overlap between groups (Fig.~\ref{fig:latent_immune_structure}B).

A co-occurrence matrix links learned clusters to reported exposure histories (Fig.~\ref{fig:latent_immune_structure}C).
...
To interpret our learned pseudo-escape values, we use human serological data from Jian et al \cite{Jian2023}.

To assess whether pseudo-escape captures the notion of similar in titer, we compute titer distances as average log2 differences in titer values between pairs of variants.
We compare these titer distances to distances in our pseudo-escape space (Fig.~\ref{fig:latent_immune_structure}D), finding the distances between distinct pairs in the pseudo-escape space are correlated with these titer differences between variants ($R^2$ = 0.402).
...
Next, we subset by exposure history and find a similar relationship in most cohorts with the distances being poorly explained for individuals with primary XBB infection and primary JN.1 infection (Fig.~\ref{fig:titer_distance_correlations_by_group}).
...
In particular, our latent factor model is most easily compared to the approaches of Meijers et al. and Raharinirina et al. which use cross-neutralization and deep mutational scanning data (respectively) to forecast variant fitness \cite{Meijers2023, Raharinirina2025}.
Our approach differs significantly in that our model does not require any data other than sequences counts for each variant over time, enabling real-time analysis of fitness and heterogeneity in population immunity before cross-neutralization and deep mutational scanning data are available.
