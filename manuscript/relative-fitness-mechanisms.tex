...
Our approach shows that relative fitness is often dependent on the past exposure of a population (as discussed in Supplemental Appendix \ref{ssec:frequency_dynamics}).
...
Later, we apply this model to data from 50 US states and England from 2021 to 2022 to estimate relative fitness for variants circulating in that period.
...
\tbc{This reads like a note to yourself. You need to significantly expand this and reference evidence in the form of figures or Supplemental Text.}
...
Using case and sequence data from 50 US states between May 2021 and November 2022, we estimate epidemic growth rates in these states and as well as selective pressure using our approximate Gaussian model on sequence counts grouped at the level of Nextstrain clades (Figs.~\ref{fig:selective_pressure_midwest}--\ref{fig:selective_pressure_west}). 
...
We validate this method by testing this model to estimate epidemic growth rates in Englad as derived ONS data between February 2022 and November 2022.
Our model is able to replicate patterns seen in epidemic growth rates in England derived from ONS data (Fig.~\ref{fig:selective_pressure_prediction} J--L), achieving a Spearman correlation of $R^2 = 0.254$ and mean absolute error of 0.02.
...
% MF: What is the relevant citation here? You directed me to this data, so I'd appreciate a bit of help here.
...
\begin{figure}[h]
    ...
    \caption{
      ...
      A. Variant frequency estimated using the Gaussian process relative fitness model between June 2021 and November 2022.
      ...
      %MF: Black here is ONS Prevalence / cases, Gray is the epidemic growth rate from data. The regressor is green.
    }
...
We, then, compare these distances to distances in our pseudo-escape space (Fig.~\ref{fig:latent_immune}G), finding the distances in the pseudo-escape space are correlated with these titer differences between variants ($R^2$ = 0.67).
...
This enables a more detailed picture of variant success in environments with heterogenous population immunity.
...
We take sequence count and case count data from all states in the United States between July 2021 and November 2022.
...
We use a gradient boosting regressor model with 50 estimators and max tree depth of 5 which is fit using a mean absolute error loss function.
This model was selected as it achieved the minimal error via time series cross-validation averaged across 10 splits among candidate models.
...
\subsection{Exponentially growing populations to frequency dynamics}\ref{ssec:frequency_dynamics}
