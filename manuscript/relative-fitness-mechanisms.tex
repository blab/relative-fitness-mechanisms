...
\begin{figure}[h]
    ...
    \caption{
      ...
      A. Relative fitness for a transmissibility increasing variant $T$ with $\varTransmission_T=0.2$ and an immune escaping variant $E$ with $\varEscape_E=0.3$ for $\beta=0.875$.
...
High fitness variants rising from initially low frequencies leads to large increases in the variance of the fitness distribution and therefore increases in the selective pressure.
\tbc{Maybe reference Fisher's theorem here.}

The selective pressure metric enables us to decompose changes in the average growth rate in the population $d\bar{r}/dt$ to an evolutionary component $\psi$ and a residual baseline growth rate $r_0$ following
\begin{equation}
...
\begin{align}
    ...
    \frac{I(t)}{I(0)} &= \exp \left( \int_0^t [r_0(s) + \Psi(s)]ds \right).
\end{align}
\tbc{Is $r_0(t)$ exponential growth rate of wildtype (pivot) variant? The only way I've so far seen $r$ subscripted is $r_v(t)$. If this is the case, I'd use something like $r_W(t)$ instead to be consistent with earlier use.}
\tbc{Also what's up with the brackets in $[r_0(t) - r_0(0)]$? Is this equivalent to $(r_0(t) - r_0(0))$? If so, can the brackets just be dropped in Equation 5?}
In addition to estimating the relative fitness, metrics derived from these models can be informative of much more.
Our ``selective pressure'' metric $\psi(t)$ allows us to model the contribution of evolution to changes in the epidemic growth rate of a population.
\tbc{Or is the ``selective pressure'' metric cumulative selection pressure $\Psi(t)$?}
This metric acts as an early warning system for variant-driven outbreaks, especially in scenarios where case data are under-reported or delayed.
This metric can be computed using any method that estimates variant frequency and relative fitness and serves as a simple tool for understanding the contribution of selection to the overall population dynamics.
...
Here we group variants at the granularity of Nextstrain clades \cite{aksamentov2021nextclade} resulting in XXX distinct variants over this time period.
\tbc{Fill in XXX with variant count, including `other' variant in the total.}
...
\begin{figure}[h]
    ...
    \caption{
      ...
      \tbc{Label panels A and C with `Washington' as well.}
      ...
      \tbc{I think the dashed line got accidently moved to the end of the time series? I suggest actually  dropping the vertical dashed lines and instead relying on lighter / darker background shading to denote training vs test split.}
      ...
      \tbc{Same statment about slightly increasing font size of tick labels.}
    }
