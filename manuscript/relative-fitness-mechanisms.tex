...
\begin{figure}[h]
    ...
    \caption{
      ...
      (A-B) Principal component analysis of subject-level titers, colored by learned immune clusters (B) and subject infection history (C).
...
% TODO: Add line about co-occurence
...
We quantified which immune clusters are preferentially targeted by circulating lineages using the model-predicted escape $\eta_{v,\cdot}$ for each variant $v$ to predict titers against that variant for individuals in each immune cluster (Fig.~\ref{fig:latent_immune_structure}D).
