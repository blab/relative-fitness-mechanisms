...
\title{Transmission mechanisms and selective pressure shape viral fitness and growth}
...
\begin{abstract}
The COVID-19 pandemic saw variants drive large waves of infections, fueled by increased transmissibility and immune escape.
Current models focus on frequency changes without linking them to transmission mechanisms.
We introduce a framework connecting variant dynamics to these mechanisms, showing how shifts in population immunity determine whether transmissibility or immune escape drives spread.
...
We also introduce a selective pressure metric that provides an early signal of epidemic growth using genetic data alone, crucial when case numbers are scarce.
Additionally, a latent immunity space model approximates immunological distances, offering insights into population susceptibility and immune evasion.

By introducing this mechanistic framework and novel metrics, we advance understanding of viral evolution and transmission dynamics. These insights refine real-time forecasting and lay the groundwork for research into the interplay between viral genetics, immunity, and epidemic growth, offering strategies to mitigate viral threats.
\end{abstract}
...
While these repeated large waves have occurred with the emergence of novel variants, the mechanism driving these variants' transmission has changed over time.
Early mutations and viral variants such as Alpha, Beta, Gamma, Delta and their associated infection waves were largely driven by increases in intrinsic transmissibility.
...
This rapid viral population turnover is consistent with antigenic evolution and is observed in other viruses such as seasonal influenza, although SARS-CoV-2 currently remains an outlier in terms of pace of its evolution \cite{Koel2013, Bedford2014, kistler2023atlas}.
This transition from transmissibility-driven to immune escape-driven variant success underscores the dynamic interplay between viral evolution and population immunity.
...
Several models of variant frequency have been developed to estimate the fitness of emerging SARS-CoV-2 variants \cite{susswein2023leveraging, figgins2022sars, Piantham2022, Annavajhala2021, Lefrancq2023}.
...
By modeling both intrinsic transmissibility and immune escape, we explain how shifts in population immunity shape the relative fitness of viral variants and selects for immune escape over intrinsic transmissibility with increasing past exposure.
...
We, therefore, present a novel non-parametric method for estimating time-varying fitness regardless of the underlying transmission mechanism.
...
Using this metric alongside historical case data, we develop a method for predicting case growth rates from sequence counts alone. 
This is especially useful when case numbers are sparse or absence, as it can be used to give an early signal of variant-driven infection waves before widespread transmission occurs.
...
In order to better understand frequency dynamics of pathogens with multiple co-circulating variants, we can apply the above framework to compartmental models of epidemics which can be written as time-varying exponential growth (Equation~\ref{eq:inhomo_exp_growth}).
...
To understand the fitness trade-off between an increased transmissibility variant $\varT$ and immune escape variant $\varE$, we consider dynamics with one wildtype and two variant viruses $v= \wt, \varE, \varT$ such that $\eta_{T}^{\varE} = 1$ and $\eta_{E}^{\varT} = 0$.

Following equation \ref{eq:two_strain_relative_fitness}, we can write relative fitnesses of the escape only variant or transmissibility increased variant as
\begin{align*}
...
We develop a scalable approximate Gaussian process model for fitness estimation which captures time-varying variant fitness with improved computational efficiency. This technique simplifies the traditional cubic complexity of GPs by using shared basis functions across variants, making the framework scalable for large datasets and long time periods.

%TODO: Sentence about practical impact here.

Additionally, we introduce a latent factor model which decomposes variant fitness into pseudo-escape rates and pseudo-immunity proportions. This allows for a deeper understanding of immune dynamics and how they shape variant success. The latent factor model, applied to SARS-CoV-2 data across several countries, successfully captures these immune escape dynamics and their impact on variant fitness.

The model also addresses geographic and temporal variation in viral fitness, recognizing that fitness can differ across regions due to variations in population immunity.
From this geographic and temporal variation, we can learn a structured immune space that is consistent with human titer data.

By incorporating these regional differences into the framework, we can provide more tailored public health strategies to mitigate variant spread.
...
This metric allows us to predict the epidemic growth rate using sequence counts alone and
By integrating fitness estimates and variant frequency dynamics, this metric acts as an early warning system, empowering public health agencies to anticipate and respond to new variant-driven outbreaks. This metric has been validated using data from both the United States and the United Kingdom.
...
While this framework has been applied to SARS-CoV-2, it is broadly generalizable to other rapidly evolving pathogens, including influenza, seasonal coronaviruses, and zoonatic viruses. 

%TODO: Clean up
The combination of genetic data, immune escape, and fitness estimation makes the framework a powerful tool for pandemic preparedness and public health surveillance. 
This approach provides real-time insights into viral evolution and can inform decisions about vaccine strategy, resource allocation, and policy interventions
