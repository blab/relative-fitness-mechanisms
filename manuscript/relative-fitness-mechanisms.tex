...
It has been shown that these transmission advantages differ geographically and temporally, suggesting that variant transmission advantages are not necessarily fixed and may be informed by regional population differences \cite{figgins2022sars, vanDorp2022}.
...
Existing models that allow variant transmission advantages to change in time often do not have a mechanistic underpinning for why transmission advantages exist and vary geographically and temporally \cite{figgins2022sars, susswein2023leveraging}.
Models that do include mechanistic grounding of transmission based on population immunity such as Meijers et al.\ \cite{Meijers2023} and Raharinirina et al.\ \cite{Raharinirina2025} have parameterized variant-specific immunity based on serological measurements or deep mutational scanning datasets.
Timely serological data is thus a requirement for these models to perform well for real-time evolutionary forecasting.

In response to this gap, we introduce a novel framework that links variant dynamics directly to transmission mechanisms using compartmental models of infectious diseases (Fig.~\ref{fig:vis_mechanisms-sketch}).
...
Alongside this development we introduce a ``selective pressure'' metric that quantifies the impact of variant turnover on population-level epidemic growth rates.
Finally, we develop a latent immunity model that we use to estimate the underlying proportion of pseudo-immune groups within multiple geographies and pseudo-immune escape rates for circulating variants that predicts antigenic distances using sequence data alone.
...
\begin{figure}[h]
    ...
    \caption{
      ...
      (D) We use mechanistic transmission models to infer relative fitness from frequency data alone, taking advantage of known structure in transmission dynamics.
      This enables us to quantify trade-offs between variant phenotypes and develop new methods for estimating fitness in populations undergoing antigenic evolution.
    }
...
Our approach overcomes this using a Hilbert Space Gaussian Process (HSGP) approximation, making the framework scalable for many variants and long time periods \cite{riutortmayol2022practical}.
...
\subsection*{Determining the transmissibility-escape tradeoff}
...
\end{align}
where $\beta$ is the transmissibility coefficient, $\varEscape$ is escape proportion against the wildtype and $\varTransmission$ is the variant's proportional increase to transmissibility.
...
To elucidate these factors, we develop and implement a method for latent factor analysis of relative fitness from sequence data alone.
...
Using these sequence counts, we apply our latent factor model to estimate the relative fitness of each variant over time in each country, pseudo-escape rates for each variant, and pseudo-immunity for each country simultaneously for $D=8$ pseudo-immune groups (Fig.~\ref{fig:latent_immune}).
...
\begin{figure}[h]
    ...
    \caption{
      ...
      We fit our latent immunity factor model for $D=8$ pseudo-immune groups using only SARS-CoV-2 sequence count data.
      ...
      B. Estimated relative fitness for selected variants and countries. These variant-specific relative fitnesses are similar across countries, but not identical.
      ...
      F. Estimated pseudo-escape rates for each variant relative to pivot variant \tbc{insert pivot variant name here}.
      \tbc{A little strange that the blue/red color ramp doesn't have a scale bar attached to it.}
    }
    \label{fig:latent_immune}
\end{figure}

Our results show that closely related Pango lineages are often assigned similar pseudo-escape values.
This is visible as a clustering of lineages with similar colors into similar coordinates in Fig.~\ref{fig:latent_immune}D-E suggesting our pseudo-escape values broadly align with evolutionary structure.
...
To examine how the learned immune structure relates to subject-level serology, we projected titers from Jian et al.\ \cite{Jian2023} into principal-component space then performed clustering with $k$-means, arriving at $k=5$ clusters using the elbow method (Fig.~\ref{fig:choosing_k_k_means}).
Our learned immune clusters yields clear separation (Fig.~\ref{fig:latent_immune_structure}A) in PCA-space, whereas as a subset of subject-level exposure histories show overlap in titer measurements (Fig.~\ref{fig:latent_immune_structure}B).
This overlap can be seen in a co-occurrence matrix linking learned clusters to reported exposure histories (Fig.~\ref{fig:latent_immune_structure}C).
These groups split and aggregate multiple exposure histories.
For example, individuals with a BA.5/BF.7 breakthrough infections split into both clusters 1 and 2, and cluster 3 contains individuals with XBB infection, JN.1 infection, and wildtype vaccine.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\linewidth]{./figures/latent-immune-structure.png}
    \caption{
      ...
      (A-B) Principal component analysis of subject-level titers, colored by learned immune clusters (A) and subject infection history (B).
      (C) Co-occurrence matrix between learned immune clusters and infection histories.
      (D) Comparing pairwise distance between variants in the pseudo-immune space to observed distances in human titer data.
      (E-F) Correlation between log mean titer against a variant and that variant's pseudo-escape by group.
      (G) Estimated escape burden by immune cluster, showing variant escape potential against individual immune clusters.
    }
    \label{fig:latent_immune_structure}
\end{figure}

We next relate measured serological titers from Jian et al.\ \cite{Jian2023} to our estimates of pseudo-escape that derive solely from variant frequency dynamics across countries.
To assess whether pseudo-escape captures titer measurements, we compute serological titer distances as average log2 differences in titer values between pairs of variants.
We compare these titer distances to distances in our pseudo-escape space (Fig.~\ref{fig:latent_immune_structure}D), finding the distances between distinct variant pairs in the pseudo-escape space are correlated with these titer differences between variants ($R^2$ = 0.402).
...
Next, we subset by exposure history and find a similar relationship in most cohorts with the stronger correlations between serological titer distance and pseudo-immune escape in general being earlier exposures (WT, BA.5) and a weaker correlations observed in cohorts with later exposures (JN.1 and XBB) (Fig.~\ref{fig:titer_distance_correlations_by_group}).

We quantified which immune clusters are preferentially targeted by circulating lineages using the model-predicted pseudo-escape $\eta_{v,\cdot}$ for each variant $v$ to predict titers against that variant for individuals in each immune cluster and exposure history group (Fig.~\ref{fig:latent_immune_structure}E-F).
This relationship is delineated in Methods section `Regression of pseudo-escape onto neutralization titers'.

To assess variant-level escape against each cluster, we predict an escape burden as the negative of the predicted titer as a proxy for the escape potential against individuals of a certain background.
Variants within JN.1 clade show elevated escape burden in specific clusters, with immune cluster~4 standing out as the most broadly targeted.
...
Further, we find that immune cluster 4 shows a strong negative escape burden again viruses in clade 23B, likely owing the fact that individuals immune cluster 4 corresponds to a subset of individuals with past XBB infection (Fig.~\ref{fig:latent_immune_structure}G).
This is further supported by the fact that among all infection histories, pseudo-escape best predicts titers in individuals with past XBB infection including those with BA.5/BF.7 breakthrough infection (Fig.~\ref{fig:titer_pseudo_escape_correlations_by_immune_group}-\ref{fig:titer_pseudo_escape_correlations_by_immune_cluster}).

In short, this shows that a sequence-only latent immunity model can recover an antigenic geometry that agrees with human serology and that learned pseudo-escape values can be used to predict cohort-level neutralization patterns.
These properties make the latent representation useful for analyzing population susceptibility and explaining geographic variation in variant success, even when contemporaneous titer data are unavailable.

This approach can be applied to other antigenically variable pathogens, such as seasonal influenza, making it broadly applicable beyond SARS-CoV-2.
...
In particular, our latent factor model is most easily compared to the approaches of Meijers et al.\ \cite{Meijers2023} and Raharinirina et al.\ \cite{Raharinirina2025} that use cross-neutralization and deep mutational scanning data respectively to parameterize variant fitness.
However, our approach differs significantly in that our model does not require any data other than sequence counts for each variant over time, enabling real-time analysis of fitness and heterogeneity in population immunity before cross-neutralization and deep mutational scanning data are available.
...
Our models, while powerful in estimating short-term variant dynamics, rely on assumptions about transmission mechanisms that may not always hold across different pathogens or contexts.
...
This approximation allows us to share basis functions between variants \cite{riutortmayol2022practical}.
...
\paragraph{Regression of pseudo-escape onto neutralization titers}

To assess the relationship between our estimated pseudo-escape values and empirical neutralization titers, we use human titer data from Jian et al.~\cite{Jian2023}.
For each exposure group, we have a set of neutralization titers measured against multiple variants.
Our goal is to determine whether variation in titers across variants can be explained by the pseudo-escape values inferred from our latent immune factor model.

Let $t_{i,v}$ denote the neutralization titer of individual $i$ against variant $v$.
For each exposure group $G$, we first aggregate titers by computing the group-level mean and applying a log$_2$ transformation with a pseudocount of one, i.e.
\begin{equation}
    y_{v}^G = \log_2\left(\mathrm{mean}_{i \in G}(t_{i,v}) + 1\right).
\end{equation}
Let $\varEscape_v \in \mathbb{R}^D$ denote the pseudo-escape vector of variant $v$ estimated from the latent immune factor model across $D$ latent factors.
We then fit an ordinary least squares (OLS) model separately for each exposure group,
\begin{equation}
    y_{v}^G = \beta_0 + \boldsymbol{\beta}^\top \varEscape_v + \varepsilon_{g,v},
\end{equation}
where $\beta_0$ is an intercept term and $\boldsymbol{\beta}$ is a vector of regression coefficients mapping latent factors to predicted group-level titers.
This yields a coefficient of determination $R^2_g$ for each group, quantifying how well variation in pseudo-escape explains variation in group-level titers.

To assess statistical significance, we performed a permutation test in which the association between variants and their pseudo-escape vectors was randomly permuted 1,000 times.
For each permutation, we re-fit the OLS model and recorded the resulting $R^2_g$, yielding a null distribution under the hypothesis that pseudo-escape and titers are unrelated.
The empirical $p$-value for each group was computed as the fraction of permutations with $R^2_g$ greater than or equal to the observed $R^2_g$.
This procedure controls for the correlation structure in the titer data while directly testing whether the inferred pseudo-escape values have predictive power for neutralization measurements.

We summarize the results across groups by reporting both the observed $R^2$ values and the permutation-derived $p$-values (Fig.~\ref{fig:titer_pseudo_escape_correlations_by_immune_group} and \ref{fig:titer_pseudo_escape_correlations_by_immune_cluster}).
This analysis identifies exposure groups where pseudo-escape provides a statistically significant explanation of the neutralization response profile.
...
\begin{align}
    \frac{d S}{d t} &= - \beta S I_{\wt} - \beta (1+\varTransmission) S I_{v},\\
...
\begin{align}
    \frac{d S}{d t} &= - \beta S I_{\wt} - \sum_{i} \beta (1+\varTransmission_i) S I_{v_{i}}\\
    \frac{d I_{\wt}}{dt} &= \beta S I_{\wt} - \gamma I_{\wt}\\
    \frac{d I_{v_{i}}}{dt} &= \beta (1+\varTransmission_i) S I_{v_{i}} + \beta (1+\varTransmission_i) \varEscape_i \phi_{\wt} I_{v_{i}} - \gamma I_{v_{i}}\\
    \frac{d \phi_{\wt}}{dt} &= \gamma I_{\wt} - \sum_{i} \beta (1+\varTransmission_i) \varEscape_i \phi_{\wt} I_{v_{i}}\\
...
Converting this estimate to an estimate of transmission advantage (relative effective reproduction number) requires assuming a Dirac delta distribution of the generation time \cite{Wallinga2006}.
...
\begin{figure}[h]
    ...
    \textbf{Estimating relative fitness with Gaussian processes.}
      ...
      C. Posterior relative fitnesses. Intervals show the 80\% credible interval. Dashed line shows true relative fitnesses from underlying mechanistic model.
    }
...
\begin{figure}[t!]
    ...
    \caption{
        ...
        This analysis uses only titer data from Jian et al.\ \cite{Jian2023}.
        \tbc{The plot title is cut-off. You just need to reduce the font size.}
    }
...
\begin{figure}[t!]
    ...
    \label{fig:titer_pseudo_escape_correlations_by_immune_group}
\end{figure}
...
\begin{figure}[t!]
    ...
    \label{fig:titer_pseudo_escape_correlations_by_immune_cluster}
\end{figure}
