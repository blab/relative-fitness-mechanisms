...
In this model, the variant virus can infect both susceptible individuals $S$ and individuals previous infected with the wild-type virus ($\phi_{\wt}$).
...
We can also think of the two-strain SIR with $\eta_{T} = 1$ as a special case in for this model where we set $S = \phi_{S}, \eta_{\wt, S} = 1, \eta_{\wt, \wt} = 0, \eta_{v, S} = 1, \eta_{v, \wt} = \eta_{E}$ and keep all other parameters the same.
