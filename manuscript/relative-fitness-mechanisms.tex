...
Using estimated selective pressure from the approximate Gaussian process model (reference section), we predict the epidemic growth rate using the selective pressure for the previous (window size) days.
We fit a (model type) model to data from (insert number) states in the United States between (start date) and (end date).
Our model is able to replicate patterns seen in estimates of the epidemic growth rates in England derived from ONS data.
...
We apply this method to sequence count data at the level of Pango lineage for X countries over the period of March 2023 to March 2024. 
...
We find that multi-strain mechanistic models of epidemics can be used to inform variant turnover at the level of frequency.
...
\paragraph{Predicting selective pressure from growth rate}%
...
DETAILS ON DATA WE APPLIED IT TO

DETAILS ON TITER DATA
