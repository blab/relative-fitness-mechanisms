...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{../notebooks/figures/gp_example.png}
    ...
    C. Relative fitneses. Dashed line shows true relative fitnesses from mechanistic model. 
}
...
Repeating this analysis for two variants $v= \varE, \varT$ such that $\eta_{T}^{\varE} = 1$ and $\eta_{E}^{\varT} = 0$, we can write relative fitnesses an escape only variant or transmissibility increased variant as
\begin{align*}
...
This trade off ... [include description and what this means] \ref{fig:transmission_tradeoff}
...
Basically, we want to also monitor decrease in absolute fitness cuz increased selection pressure =/= increased cases
