...
We observe a strong correspondence between observed epidemic growth rate and model predictions with Pearson $R^2$ in the training period of XXX and Pearson $R^2$ in the testing period of XXX.
...
These 14 countries were chosen based on availability of sequence data.
...
We compare these distances to distances in our pseudo-escape space (Fig.~\ref{fig:latent_immune}G), finding the distances between distinct pairs in the pseudo-escape space are correlated with these titer differences between variants ($R^2$ = 0.26).
We bootstrap this analysis among 1000 replicates to assess signficance of this correlation (Fig.~\ref{fig:latent_immune_bootstrap}, $p = $).
...
\begin{figure}[h]
    ...
    \caption{
      ...
      A. Variant frequency. Lines are colored to show 4 variants of interest (of 41 total variants) with the style of the line denoting 3 countries of interest (of 14 total countries).
      ...
      \tbc{Currently the colors in panels D and E are meaningful but it's hard to convey that. You could do so by coloring the clade and lineage label text at the bottom and top of the matrix by these same colors.}
      \tbc{It would be nice if these clade and lineage labels were larger. I struggle to read them at the moment.}
...
This is accomplished using a multinomial likelihood, so that given count of sequences $C_{t,v}$ of variant $v$ at time $t$ and total sequences $N_{t}$ collected at time $t$, we have that
\begin{equation*}
    C_{t, \cdot} \sim \text{Multinomial}(N_{t}, f_{v}(t)),
\end{equation*}
where $f_{v}(t)$ is the frequency of variant $v$ at time $t$.
\tbc{I'm a little confused on why this isn't $N(t)$ and $C_v(t)$. I think it's because it's fundementally an array of observations rather than a continuous variable, but the differing subscripting is still confusing to me. It's okay if you feel it's best to keep it as it stands, but just noting my surprise.}
\tbc{Also, I'd think with $f_{v}(t)$ in the equation you want $C_{t, v}$ on the left rather than the existing $C_{t, \cdot}$.}
...
\tbc{Can we be more consistent in notation for base or pivot variant? It's $*$ here, but $\mathrm{pivot}$ in Equation 1. What about just using $*$ throughout?}
For the same reason, we fix the order of components, so that the components are numbered in decreasing order by their share in the a arbitrarily defined base geography.
...
\begin{figure}[t!]
    ...
    \caption{
      ...
      % MF: Yes, this was a random matplotlib export error! Fixed.
    }
